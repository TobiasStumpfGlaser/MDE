...
pub struct EncApRepPart {
    /// Client time copied from the AP-REQ Authenticator.
    pub ctime: String,
    /// Client microseconds copied from the AP-REQ Authenticator.
    pub cusec: u32,
...
pub fn parse_enc_ap_rep_part(data: &[u8]) -> Result<EncApRepPart, Error> {
    ...
    for (ftag, fvalue) in &fields {
        match ftag {
            0xa0 => ctime = Some(parse_der_generalized_time(fvalue)?),
            0xa1 => cusec = Some(parse_der_integer_u32(fvalue)?),
    ...
    Ok(EncApRepPart {
        ctime: ctime
            .ok_or_else(|| Error::invalid_data("Kerberos: missing ctime in EncAPRepPart"))?,
        cusec: cusec
            .ok_or_else(|| Error::invalid_data("Kerberos: missing cusec in EncAPRepPart"))?,
        subkey,
        seq_number,
    })
}
...
mod tests {
    ...
    #[test]
    fn test_parse_enc_ap_rep_part_keeps_client_time() {
        let ctime = der_context(0, &der_generalized_time("20260408120000Z"));
        let cusec = der_context(1, &der_integer_u32(123_456));
        let seq_number = der_context(3, &der_integer_u32(42));
        let sequence = der_sequence(&[&ctime, &cusec, &seq_number]);
        let encoded = der_application(27, &sequence);

        let parsed = parse_enc_ap_rep_part(&encoded).unwrap();
        assert_eq!(parsed.ctime, "20260408120000Z");
        assert_eq!(parsed.cusec, 123_456);
        assert_eq!(parsed.seq_number, Some(42));
        assert!(parsed.subkey.is_none());
    }

    #[test]
    fn test_parse_enc_ap_rep_part_requires_client_time() {
        let sequence = der_sequence(&[]);
        let encoded = der_application(27, &sequence);

        assert!(parse_enc_ap_rep_part(&encoded).is_err());
    }

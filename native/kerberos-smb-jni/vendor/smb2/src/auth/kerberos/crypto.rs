...
pub fn string_to_key_aes(password: &str, salt: &str, key_size: usize) -> Vec<u8> {
    string_to_key_aes_with_iterations(password, salt, key_size, 4096)
}

/// Derive an AES encryption key using KDC-provided string-to-key parameters.
///
/// RFC 3962 encodes the PBKDF2 iteration count in the optional `s2kparams`
/// field of `ETYPE-INFO2`. When the field is absent, callers should use the
/// default value of 4096 via [`string_to_key_aes`].
pub fn string_to_key_aes_with_iterations(
    password: &str,
    salt: &str,
    key_size: usize,
    iterations: u32,
    ...
    // Step 1: PBKDF2-HMAC-SHA1 with the negotiated iteration count.

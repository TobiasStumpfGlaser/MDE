...
impl Session {
    ...
    async fn setup_kerberos_with_auth(
        ...
        {
            // Authentication failures can carry a SPNEGO response containing
            // a precise KRB-ERROR. Inspect both legal SMB body layouts before
            // falling back to the generic NTSTATUS.
            let mut security_buffers = Vec::new();
            let mut setup_cursor = ReadCursor::new(&resp_body);
            if let Ok(setup_response) = SessionSetupResponse::unpack(&mut setup_cursor) {
                if !setup_response.security_buffer.is_empty() {
                    security_buffers.push(setup_response.security_buffer);
                }
            }
            let mut error_cursor = ReadCursor::new(&resp_body);
            if let Ok(error_response) = ErrorResponse::unpack(&mut error_cursor) {
                if !error_response.error_data.is_empty() {
                    security_buffers.push(error_response.error_data);
                }
            }

            for security_buffer in security_buffers {
                if let Ok(spnego_response) =
                    crate::auth::spnego::parse_neg_token_resp(&security_buffer)
                {
                    if let Some(token_bytes) = spnego_response.response_token {

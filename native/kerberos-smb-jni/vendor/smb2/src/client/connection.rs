...
impl Connection {
    ...
    pub async fn connect(addr: &str, timeout: Duration) -> Result<Self> {
        ...
        Self::connect_named(addr, &server_name, timeout).await
    }

    /// Connect to an SMB endpoint while using a separate logical server name
    /// for UNC paths. This is required when a short Windows server name is
    /// resolved through a DNS alias but Kerberos and DFS use the short name.
    pub async fn connect_named(addr: &str, server_name: &str, timeout: Duration) -> Result<Self> {
        if server_name.is_empty() {
            return Err(Error::invalid_data("server name must not be empty"));
        }
